...
from pipeline import ProtectedRAG, rag
from config import load_settings
ServiceError, OutputError, format_reply = rag.ServiceError, rag.OutputError, rag.format_reply


class TelegramRAG(ProtectedRAG):
    def ask(self, question):
        result = super().ask(question, mode='all')
...
def main():
    ...
    if not token or token == 'PUT_TOKEN_HERE':
        raise SystemExit('Set TELEGRAM_BOT_TOKEN in task-5/.env')
    try:
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit(f'Bot stopped: {type(exc).__name__}. Check Ollama, task-5/index and the local token.')

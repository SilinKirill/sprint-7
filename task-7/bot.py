...
from logging7 import open_session
from shared7 import rag
from settings7 import load_settings
ServiceError, OutputError, format_reply = rag.ServiceError, rag.OutputError, rag.format_reply


class TelegramRAG:
    def __init__(self, settings):
        self.session = open_session(os.getenv('EVALUATION_VARIANT', 'gaps'), settings)

    def ask(self, question):
        result = self.session.ask(question)
        if result['status'] == 'error':
            raise OutputError(result['error'])
        return result
...
def main():
    ...
    if not token or token == 'PUT_TOKEN_HERE':
        raise SystemExit('Set TELEGRAM_BOT_TOKEN in task-7/.env')
    try:
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit(f'Bot stopped: {type(exc).__name__}. Check Ollama, task-7/index and the local token.')

...
def main():
    ...
    if not token or token == 'PUT_TOKEN_HERE':
        raise SystemExit('Set TELEGRAM_BOT_TOKEN in task-6/.env')
    try:
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit(f'Bot stopped: {type(exc).__name__}. Check Ollama, task-6/index and the local token.')
